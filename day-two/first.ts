...
export function main(comparator: string, input: string): number {
    const maxAllowable = parseColorAndAmounts(comparator)
    return input.split('\n').reduce((sum, str) => {
        let gameData = getGameData(str);
        let colorMaxxes = getColorMaxxes(gameData.draws);
        const isPossible = Object.keys(colorMaxxes).reduce((isValid, color) => {
            return isValid && colorMaxxes[color] <= maxAllowable[color]
        }, true)
        if(isPossible) {
            return sum + gameData.gameId
        }
        return sum
    }, 0)
}
